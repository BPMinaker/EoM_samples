...
# define a main function that will run the analysis
function main()
    ...
    # here, we make the Bode (i.e., frequency response) plot using the second, third, and fourth ouputs (kx, cxot, mxddot) only, because the Bode plot should be dimensionless, i.e., input and output should have the same units, so we plot the ratio of spring force to applied force, ratio of damping force to applied force, and the ratio of inertial force to applied force, as a function of frequency; the spring force is proportional to displacement, so we are really looking at displacement response, but in a nondimensional way; we should see a resonance near the natural frequency, as long as the damping ratio is below 0.707; note that at low frequencies, the spring force will nearly equal the applied force, so the Bode plot will tend toward 1.0, or 0 [dB] (remember the decibel is a logarithmic unit); at high frequency, the applied force changes direction so quickly that the mass doesn't have time to respond, so the motion becomes very small, i.e., 0.0 or -∞ [dB], but the applied force will nearly equal the inertial force, which will tend toward 0 [dB]
